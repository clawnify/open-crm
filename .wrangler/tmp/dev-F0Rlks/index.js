...
// node_modules/.pnpm/wrangler@4.93.1_@cloudflare+workers-types@4.20260702.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
...
// node_modules/.pnpm/wrangler@4.93.1_@cloudflare+workers-types@4.20260702.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
// node_modules/.pnpm/hono@4.12.2/node_modules/hono/dist/utils/cookie.js
...
// node_modules/.pnpm/hono@4.12.2/node_modules/hono/dist/helper/cookie/index.js
...
// node_modules/.pnpm/hono@4.12.2/node_modules/hono/dist/http-exception.js
...
// node_modules/.pnpm/hono@4.12.2/node_modules/hono/dist/utils/buffer.js
...
// node_modules/.pnpm/hono@4.12.2/node_modules/hono/dist/validator/validator.js
...
// node_modules/.pnpm/@hono+zod-validator@0.4.3_hono@4.12.2_zod@3.25.76/node_modules/@hono/zod-validator/dist/index.js
...
// node_modules/.pnpm/@hono+zod-openapi@0.18.4_hono@4.12.2_zod@3.25.76/node_modules/@hono/zod-openapi/dist/index.mjs
...
// node_modules/.pnpm/@clawnify+integrations@0.2.2/node_modules/@clawnify/integrations/dist/index.js
var DEFAULT_INSIGHT_FIELDS = ["spend", "impressions", "clicks", "actions", "action_values"];
function client(ctx) {
  return {
    async adAccounts(fields = "id,name,currency,account_status") {
      const d = await ctx.run("METAADS_GET_AD_ACCOUNTS", { fields, limit: 100 });
      return d?.data ?? [];
    },
    async insights(objectId, opts = {}) {
      const args = {
        object_id: objectId,
        level: opts.level ?? "account",
        fields: opts.fields ?? DEFAULT_INSIGHT_FIELDS
      };
      if (opts.since && opts.until) args.time_range = { since: opts.since, until: opts.until };
      else args.date_preset = opts.datePreset ?? "last_30d";
      if (opts.timeIncrement != null) args.time_increment = opts.timeIncrement;
      const d = await ctx.run("METAADS_GET_INSIGHTS", args);
      return d?.data ?? [];
    },
    async object(objectId, fields) {
      const d = await ctx.run("METAADS_GET_OBJECT", { object_id: objectId, fields });
      return d ?? {};
    }
  };
}
...
var metaads = {
  ...
  label: "Meta Ads",
  kind: "oauth",
  accessor: `connect("metaads").adAccounts("id,name,currency")`,
  client
};
var SEARCH_STREAM = "GOOGLEADS_SEARCH_STREAM_GAQL";
var LIST_CUSTOMERS = "GOOGLEADS_LIST_ACCESSIBLE_CUSTOMERS";
var DEFAULT_VERSION = "v21";
...
function resultRows(data) {
  ...
  if (!d) return [];
  if (Array.isArray(d)) {
    if (d.length && (d[0]?.results || d[0]?.result)) return d.flatMap((b) => b.results ?? b.result ?? []);
    return d;
  }
...
function client2(ctx) {
  const version2 = ctx.env.GOOGLEADS_API_VERSION || DEFAULT_VERSION;
  const developerToken = ctx.env.GOOGLEADS_DEVELOPER_TOKEN;
  const loginCustomerId = (ctx.env.GOOGLEADS_LOGIN_CUSTOMER_ID ?? "").replace(/-/g, "");
  const managed = ctx.managed;
  async function directHeaders() {
    const token = await ctx.token();
    if (!token || !developerToken) throw new Error("Google Ads is not connected");
    const h = {
  ...
  __name(directHeaders, "directHeaders");
  return {
    singleCustomer: managed,
    async query(gaql, opts = {}) {
      if (managed) return resultRows(await ctx.run(SEARCH_STREAM, { query: gaql }));
      const cid = (opts.customerId ?? "").replace(/\D/g, "");
      const res = await fetch(
        `https://googleads.googleapis.com/${version2}/customers/${cid}/googleAds:searchStream`,
        { method: "POST", headers: await directHeaders(), body: JSON.stringify({ query: gaql }) }
      );
    ...
    async listCustomerIds() {
      if (managed) {
        const data2 = unwrap(await ctx.run(LIST_CUSTOMERS, {}));
        const names = data2?.resourceNames ?? data2?.resource_names ?? data2?.data?.resourceNames ?? (Array.isArray(data2) ? data2 : []);
        return (names ?? []).map((rn) => String(rn).split("/")[1] ?? String(rn)).map((s) => s.replace(/\D/g, "")).filter(Boolean);
      }
      const res = await fetch(
        `https://googleads.googleapis.com/${version2}/customers:listAccessibleCustomers`,
        { headers: await directHeaders() }
      );
      const data = await res.json();
...
__name(client2, "client2");
var googleads = {
  service: "googleads",
  label: "Google Ads",
  kind: "managed",
  accessor: `connect("googleads").query("SELECT customer.id FROM customer")`,
  client: client2
};
var openrouter = {
  service: "openrouter",
  ...
  accessor: `secret("OPENROUTER_API_KEY")`
};
var DESCRIPTORS = { metaads, googleads, openrouter };
function getDescriptor(service) {
  return DESCRIPTORS[service];
}
__name(getDescriptor, "getDescriptor");
var proxy_gen_default = {
  facebook: {
    authMode: "OAUTH2",
  ...
  bird: {
    authMode: "API_KEY",
    baseUrl: "https://${connectionConfig.subdomain}.bird.com",
    proxyHeaders: {
...
var MAP = proxy_gen_default;
function proxyConfig(service) {
  const entry = MAP[service];
  if (!entry) return void 0;
  return {
    baseUrl: entry.baseUrl,
    proxyHeaders: entry.proxyHeaders,
    authMode: entry.authMode,
    connectionConfigDefaults: entry.connectionConfigDefaults
  };
}
__name(proxyConfig, "proxyConfig");

// node_modules/.pnpm/@clawnify+connections@0.2.2/node_modules/@clawnify/connections/dist/index.js
var TEMPLATE_RE = /\$\{([^}]+)\}/g;
var BEARER_MODES = /* @__PURE__ */ new Set(["OAUTH2", "OAUTH2_CC", "OAUTH1", "JWT"]);
function fillTemplate(tpl, vars, where) {
  return tpl.replace(TEMPLATE_RE, (_m, expr) => {
    const key = expr.trim();
    if (key === "accessToken") {
      if (vars.accessToken == null) throw new Error(`${where}: no access token to fill \${accessToken}`);
      return vars.accessToken;
    }
    if (key === "apiKey") {
      if (vars.apiKey == null) throw new Error(`${where}: no api key to fill \${apiKey}`);
      return vars.apiKey;
    }
    if (key.startsWith("connectionConfig.")) {
      const field = key.slice("connectionConfig.".length);
      const v = vars.connectionConfig[field];
      if (v == null || v === "") {
        throw new Error(
          `${where}: missing connectionConfig "${field}" \u2014 pass it in opts.connectionConfig or set env ${field.toUpperCase()}`
        );
      }
      return v;
    }
    throw new Error(`${where}: unsupported template variable \${${key}}`);
  });
}
__name(fillTemplate, "fillTemplate");
function resolveRequest(service, cfg, token, env2, opts) {
  if (!cfg.baseUrl) throw new Error(`"${service}" has no base URL in the connections registry`);
  const connectionConfig = {};
  for (const [k, v] of Object.entries(opts.connectionConfig ?? {})) {
    if (typeof v === "string") connectionConfig[k] = v;
  }
  const cfgVarRe = /\$\{connectionConfig\.([^}]+)\}/g;
  for (const tpl of [cfg.baseUrl, ...Object.values(cfg.proxyHeaders ?? {})]) {
    for (const m of tpl.matchAll(cfgVarRe)) {
      const field = m[1].trim();
      if (connectionConfig[field] != null) continue;
      const fromEnv = env2[`${service.toUpperCase().replace(/-/g, "_")}_${field.toUpperCase()}`];
      if (typeof fromEnv === "string" && fromEnv) {
        connectionConfig[field] = fromEnv;
        continue;
      }
      const fromDefault = cfg.connectionConfigDefaults?.[field];
      if (typeof fromDefault === "string" && fromDefault) connectionConfig[field] = fromDefault;
    }
  }
  const vars = { accessToken: token, apiKey: token, connectionConfig };
  const base = fillTemplate(cfg.baseUrl, vars, `${service} base URL`).replace(/\/+$/, "");
  const path = opts.endpoint.startsWith("/") ? opts.endpoint : `/${opts.endpoint}`;
  let url = base + path;
  if (opts.query) {
    const qs = new URLSearchParams();
    for (const [k, v] of Object.entries(opts.query)) if (v !== void 0) qs.set(k, String(v));
    const s = qs.toString();
    if (s) url += (url.includes("?") ? "&" : "?") + s;
  }
  const headers = {};
  if (cfg.proxyHeaders && Object.keys(cfg.proxyHeaders).length) {
    for (const [name, tpl] of Object.entries(cfg.proxyHeaders)) {
      headers[name] = fillTemplate(tpl, vars, `${service} header "${name}"`);
    }
  } else if (token && BEARER_MODES.has(cfg.authMode)) {
    headers["authorization"] = `Bearer ${token}`;
  }
  let body;
  if (opts.body !== void 0 && opts.body !== null) {
    if (typeof opts.body === "string" || opts.body instanceof Uint8Array || opts.body instanceof ArrayBuffer) {
      body = opts.body;
    } else {
  ...
  return { url, method: opts.method ?? "GET", headers, body };
}
...
function makeProxyClient(service, ctx) {
  const cfg = proxyConfig(service);
  if (!cfg) return null;
  async function request(opts) {
    const token = await ctx.token();
    if (token == null && cfg.authMode !== "NONE") {
      throw new Error(
        `"${service}" has no usable credential for a direct API call. If it's a managed/Composio integration, use connect("${service}").run(action, body) (find the action via searchActions).`
      );
    }
    const { url, method, headers, body } = resolveRequest(service, cfg, token, ctx.env, opts);
    const res = await fetch(url, { method, headers, body });
    return { status: res.status, headers: res.headers, data: await parseResponse(res) };
  }
  __name(request, "request");
  async function send2(method, endpoint, opts) {
    const r = await request({ ...opts, method, endpoint });
    if (r.status < 200 || r.status >= 300) {
      const snippet = typeof r.data === "string" ? r.data : JSON.stringify(r.data);
      throw new Error(`${service} ${method} ${endpoint} \u2192 ${r.status}: ${(snippet ?? "").slice(0, 300)}`);
    }
    return r.data;
  }
  __name(send2, "send");
  return {
    request,
    get: /* @__PURE__ */ __name((endpoint, opts = {}) => send2("GET", endpoint, opts), "get"),
    post: /* @__PURE__ */ __name((endpoint, body, opts = {}) => send2("POST", endpoint, { ...opts, body }), "post"),
    put: /* @__PURE__ */ __name((endpoint, body, opts = {}) => send2("PUT", endpoint, { ...opts, body }), "put"),
    patch: /* @__PURE__ */ __name((endpoint, body, opts = {}) => send2("PATCH", endpoint, { ...opts, body }), "patch"),
    delete: /* @__PURE__ */ __name((endpoint, opts = {}) => send2("DELETE", endpoint, opts), "delete")
  };
}
__name(makeProxyClient, "makeProxyClient");
...
function clawnifyBinding(opts) {
  ...
  return {
    ...
    async searchActions(query2, service, limit) {
      const res = await fetch(`${base}/v1/connections/actions`, {
        method: "POST",
        headers,
        body: JSON.stringify({ query: query2, service, limit })
      });
      if (!res.ok) return [];
      const j = await res.json();
      return j.actions ?? [];
...
__name(clawnifyBinding, "clawnifyBinding");
function resolveBinding(env2) {
  if (env2.CREDENTIALS) return { binding: env2.CREDENTIALS, remote: false };
  const token = typeof env2.CLAWNIFY_TOKEN === "string" ? env2.CLAWNIFY_TOKEN : "";
  if (token) {
    const baseUrl = typeof env2.CLAWNIFY_API_URL === "string" ? env2.CLAWNIFY_API_URL : void 0;
    const orgId = typeof env2.CLAWNIFY_ORG_ID === "string" ? env2.CLAWNIFY_ORG_ID : void 0;
    return { binding: clawnifyBinding({ token, baseUrl, orgId }), remote: true };
  }
  return { remote: false };
}
__name(resolveBinding, "resolveBinding");
function resolveOrg(env2, orgId) {
  return orgId ?? env2.CLAWNIFY_ORG_ID ?? null;
}
__name(resolveOrg, "resolveOrg");
function buildContext(service, env2, orgId) {
  const { binding: binding2, remote } = resolveBinding(env2);
  const canBroker = !!binding2 && (remote || !!orgId);
  return {
    orgId,
    managed: canBroker,
    env: env2,
    async token() {
      if (binding2 && canBroker) {
        try {
          return await binding2.getToken(service, orgId ?? "");
        } catch {
          return null;
        }
      }
      const up = service.toUpperCase();
      const local = env2[`${up}_BEARER_TOKEN`] ?? env2[`${up}_ACCESS_TOKEN`];
    ...
    async run(action, args = {}) {
      ...
      const r = await binding2.executeTool(service, action, args, orgId ?? "");
...
function unsupportedProxy(service) {
  const fail = /* @__PURE__ */ __name(async () => {
    throw new Error(
      `"${service}" is not a registry-backed own-tier provider; the generic proxy can't reach it. Use connect("${service}").run(action, body) (find the action via searchActions).`
    );
  }, "fail");
  return { request: fail, get: fail, post: fail, put: fail, patch: fail, delete: fail };
}
__name(unsupportedProxy, "unsupportedProxy");
function connect(service, env2, orgId) {
  const ctx = buildContext(service, env2, resolveOrg(env2, orgId));
  ...
  const proxy = makeProxyClient(service, ctx) ?? unsupportedProxy(service);
  return { token: ctx.token, run: ctx.run, ...proxy };
}
__name(connect, "connect");
function secret(name, env2) {
  const v = env2[name];
  return typeof v === "string" && v.length > 0 ? v : null;
}
__name(secret, "secret");
async function isConnected(service, env2, orgId) {
  const desc = getDescriptor(service);
  if (!desc) return false;
  const org = resolveOrg(env2, orgId);
  if (desc.kind === "key" || desc.kind === "secret") {
    return !!secret(desc.envName ?? service, env2);
  }
  const { binding: binding2, remote } = resolveBinding(env2);
  if (binding2 && (remote || org)) {
    try {
      if ((await binding2.listConnected(org ?? "")).includes(service)) return true;
    } catch {
    }
  }
  return !!await buildContext(service, env2, org).token();
}
...
async function connectionStatus(env2) {
  const [email, meeting, slack] = await Promise.all([
    isConnected(SERVICES.email, env2).catch(() => false),
    isConnected(SERVICES.meeting, env2).catch(() => false),
    isConnected(SERVICES.slack, env2).catch(() => false)
  ]);
  return { email, meeting, slack };
}
__name(connectionStatus, "connectionStatus");
async function sendEmail(env2, args) {
  return connect(SERVICES.email, env2).run("GOOGLESUPER_SEND_EMAIL", {
...
async function createMeeting(env2, args) {
  return connect(SERVICES.meeting, env2).run("GOOGLECALENDAR_CREATE_EVENT", {
...
__name(createMeeting, "createMeeting");
async function notifySlack(env2, args) {
  return connect(SERVICES.slack, env2).run("SLACK_SEND_MESSAGE", {
    channel: args.channel,
    markdown_text: args.text
  });
}
__name(notifySlack, "notifySlack");
...
__name(isEntityType, "isEntityType");
var BUILTIN_COLUMNS = {
  company: /* @__PURE__ */ new Set(["id", "name", "domain", "industry", "phone", "email", "notes", "created_at", "updated_at"]),
  contact: /* @__PURE__ */ new Set(["id", "first_name", "last_name", "email", "phone", "company_id", "title", "status", "created_at", "updated_at"]),
  deal: /* @__PURE__ */ new Set(["id", "name", "contact_id", "value", "stage", "close_date", "notes", "created_at", "updated_at"])
};
var KEY_RE = /^[a-z][a-z0-9_]*$/;
function assertValidKey(entity, key) {
  if (!KEY_RE.test(key)) {
...
async function listDefs(entity) {
  const rows = entity ? await query(
    ...
    [entity]
  ) : await query("SELECT * FROM custom_field_defs ORDER BY entity_type, position, created_at");
  return rows.map(rowToDef);
}
__name(listDefs, "listDefs");
async function getDef(id) {
  const row = await get("SELECT * FROM custom_field_defs WHERE id = ?", [id]);
  return row ? rowToDef(row) : null;
}
__name(getDef, "getDef");
async function createDef(input) {
...
__name(createDef, "createDef");
async function updateDef(id, patch) {
...
async function deleteDef(id) {
  ...
  if (KEY_RE.test(def.key) && !BUILTIN_COLUMNS[def.entity_type].has(def.key)) {
    const table3 = ENTITY_TABLES[def.entity_type];
    await run(`ALTER TABLE ${quote(table3)} DROP COLUMN ${quote(def.key)}`).catch(() => {
    });
  }
  return true;
}
__name(deleteDef, "deleteDef");
function sqliteAffinity(t) {
  switch (t) {
    case "integer":
    case "boolean":
      return "INTEGER";
...
__name(sqliteAffinity, "sqliteAffinity");
async function syncEntityColumns(entity) {
  const table3 = ENTITY_TABLES[entity];
  const cols = await query(`PRAGMA table_info(${quote(table3)})`);
  ...
  for (const def of defs) {
    ...
    await run(`ALTER TABLE ${quote(table3)} ADD COLUMN ${quote(def.key)} ${sqliteAffinity(def.field_type)}`);
  }
}
__name(syncEntityColumns, "syncEntityColumns");
function coerceCustomValue(value, def) {
...
__name(coerceCustomValue, "coerceCustomValue");
...
__name(chunk, "chunk");
async function logActivity(entity_type, entity_id, type, body, meta = {}) {
  try {
    await run(
...
async function applyCustomValues(entity, table3, id, custom2) {
  if (!custom2 || typeof custom2 !== "object") return;
  const defs = await listDefs(entity);
  const byKey = new Map(defs.map((d) => [d.key, d]));
  const sets = [];
  const params = [];
  for (const [key, raw2] of Object.entries(custom2)) {
    const def = byKey.get(key);
    if (!def) continue;
    sets.push(`"${key.replace(/"/g, '""')}" = ?`);
    params.push(coerceCustomValue(raw2, def));
  }
  if (sets.length === 0) return;
  params.push(id);
  await run(`UPDATE ${table3} SET ${sets.join(", ")} WHERE id = ?`, params);
}
__name(applyCustomValues, "applyCustomValues");
var CustomValues = external_exports.record(external_exports.string(), external_exports.any()).optional();
...
var CompanySchema = external_exports.object({
  ...
  contact_count: external_exports.number().int().optional(),
...
var ContactSchema = external_exports.object({
  ...
  email: external_exports.string(),
  phone: external_exports.string(),
  company_id: external_exports.string().nullable(),
  title: external_exports.string(),
  status: external_exports.string(),
  company_name: external_exports.string().nullable().optional(),
  company_domain: external_exports.string().nullable().optional(),
  created_at: external_exports.string(),
  updated_at: external_exports.string()
}).openapi("Contact");
var DealSchema = external_exports.object({
  ...
  notes: external_exports.string(),
  contact_first_name: external_exports.string().nullable().optional(),
  contact_last_name: external_exports.string().nullable().optional(),
  company_name: external_exports.string().nullable().optional(),
  company_domain: external_exports.string().nullable().optional(),
  created_at: external_exports.string(),
  updated_at: external_exports.string()
}).openapi("Deal");
var IdParam = external_exports.object({ id: external_exports.string().openapi({ description: "Resource ID (integer)" }) });
var PaginationQuery = external_exports.object({
  page: external_exports.string().optional().openapi({ description: "Page number (default: 1)" }),
  limit: external_exports.string().optional().openapi({ description: "Items per page (default: 25, max: 100)" }),
  sort: external_exports.string().optional().openapi({ description: "Column to sort by" }),
  order: external_exports.enum(["asc", "desc"]).optional().openapi({ description: "Sort direction (default: desc)" }),
  search: external_exports.string().optional().openapi({ description: "Search term" })
});
  ...
  responses: {
    200: {
      ...
      content: { "application/json": { schema: external_exports.object({
        contacts: external_exports.number().int(),
        companies: external_exports.number().int(),
        deals: external_exports.number().int(),
        dealValue: external_exports.number()
      }) } }
    },
...
app.openapi(getStats, async (c) => {
  try {
    ...
    const companies = await get("SELECT COUNT(*) as count FROM companies");
    const deals = await get("SELECT COUNT(*) as count FROM deals");
    const dealValue = await get("SELECT COALESCE(SUM(value), 0) as total FROM deals WHERE stage NOT IN ('lost')");
    return c.json({
...
var listCompanies = createRoute({
  ...
  request: {
    query: PaginationQuery.extend({
      industry: external_exports.string().optional().openapi({ description: "Filter by industry" })
    })
  },
  responses: {
    200: {
...
var createCompany = createRoute({
  ...
  responses: {
    201: { description: "Created company", content: { "application/json": { schema: external_exports.object({ company: CompanySchema }) } } },
...
var updateCompany = createRoute({
  ...
  responses: {
    200: { description: "Updated company", content: { "application/json": { schema: external_exports.object({ company: CompanySchema }) } } },
    400: { description: "Validation error", content: { "application/json": { schema: ErrorSchema } } },
...
var listContacts = createRoute({
  ...
  request: {
    query: PaginationQuery.extend({
      status: external_exports.string().optional().openapi({ description: "Filter by status (lead, customer, etc.)" }),
      company_id: external_exports.string().optional().openapi({ description: "Filter by company ID" })
    })
  },
  responses: {
    200: {
...
var createContact = createRoute({
  ...
  request: {
    body: {
      ...
      content: { "application/json": { schema: external_exports.object({
        ...
        email: external_exports.string().optional(),
        phone: external_exports.string().optional(),
        company_id: external_exports.string().nullable().optional(),
  ...
  responses: {
    201: { description: "Created contact", content: { "application/json": { schema: external_exports.object({ contact: ContactSchema }) } } },
    400: { description: "Validation error", content: { "application/json": { schema: ErrorSchema } } },
    500: { description: "Server error", content: { "application/json": { schema: ErrorSchema } } }
  }
});
...
var updateContact = createRoute({
  ...
  responses: {
    200: { description: "Updated contact", content: { "application/json": { schema: external_exports.object({ contact: ContactSchema }) } } },
    400: { description: "Validation error", content: { "application/json": { schema: ErrorSchema } } },
    404: { description: "Not found", content: { "application/json": { schema: ErrorSchema } } },
    500: { description: "Server error", content: { "application/json": { schema: ErrorSchema } } }
  }
...
var getDealsBoard = createRoute({
  method: "get",
  path: "/api/deals/board",
  tags: ["Deals"],
  summary: "Get all deals for the pipeline board view",
  responses: {
    200: { description: "All deals with contact/company info", content: { "application/json": { schema: external_exports.object({ deals: external_exports.array(DealSchema) }) } } },
    500: { description: "Server error", content: { "application/json": { schema: ErrorSchema } } }
  }
});
...
var listDeals = createRoute({
  ...
  request: {
    query: PaginationQuery.extend({
      stage: external_exports.string().optional().openapi({ description: "Filter by stage (prospect, qualified, proposal, negotiation, won, lost)" }),
      contact_id: external_exports.string().optional().openapi({ description: "Filter by contact ID" })
    })
  },
  responses: {
    200: {
...
var createDeal = createRoute({
  ...
  request: {
    body: {
      ...
      content: { "application/json": { schema: external_exports.object({
        name: external_exports.string().min(1),
        contact_id: external_exports.string().nullable().optional(),
  ...
  responses: {
    201: { description: "Created deal", content: { "application/json": { schema: external_exports.object({ deal: DealSchema }) } } },
    400: { description: "Validation error", content: { "application/json": { schema: ErrorSchema } } },
    500: { description: "Server error", content: { "application/json": { schema: ErrorSchema } } }
  }
});
...
var updateDeal = createRoute({
  ...
  responses: {
    200: { description: "Updated deal", content: { "application/json": { schema: external_exports.object({ deal: DealSchema }) } } },
    400: { description: "Validation error", content: { "application/json": { schema: ErrorSchema } } },
    404: { description: "Not found", content: { "application/json": { schema: ErrorSchema } } },
    500: { description: "Server error", content: { "application/json": { schema: ErrorSchema } } }
  }
...
app.openapi(updateDeal, async (c) => {
  try {
    ...
    if (body.stage === "won" && updated) {
      ...
      const channel2 = c.env.SLACK_CHANNEL?.trim();
      if (channel2) {
        const contact = [updated.contact_first_name, updated.contact_last_name].filter(Boolean).join(" ");
        const text = `\u{1F389} *Deal won:* ${updated.name} \u2014 $${value.toLocaleString()}${contact ? ` (${contact})` : ""}`;
        try {
          await notifySlack(c.env, { channel: channel2, text });
          await logActivity("deal", id, "slack", `Notified #${channel2} of the win`, { channel: channel2 });
        } catch {
        }
      }
    }
...
var allCompanies = createRoute({
  ...
  responses: {
    200: {
      ...
      content: { "application/json": { schema: external_exports.object({
        companies: external_exports.array(external_exports.object({
          id: external_exports.string(),
          name: external_exports.string(),
          domain: external_exports.string()
        }))
    ...
    200: {
      ...
      content: { "application/json": { schema: external_exports.object({
        contacts: external_exports.array(external_exports.object({
          ...
          first_name: external_exports.string(),
          last_name: external_exports.string(),
          company_name: external_exports.string().nullable(),
          company_domain: external_exports.string().nullable()
        }))
      }) } }
    },
    500: { description: "Server error", content: { "application/json": { schema: ErrorSchema } } }
...
var ENTITY_TYPES = ["contact", "company", "deal"];
...
app.post("/api/integrations/meeting", async (c) => {
  try {
    ...
    const contact = await get("SELECT email FROM contacts WHERE id = ?", [contactId]);
    if (!contact) return c.json({ error: "Contact not found" }, 404);
...
var CONTACT_STATUSES = ["lead", "active", "inactive", "churned"];
var CONTACT_COLS = 8;
var CONTACT_CHUNK = Math.floor(100 / CONTACT_COLS);
var LOOKUP_CHUNK = 100;
var COMPANY_INSERT_CHUNK = 50;
app.post("/api/contacts/import", async (c) => {
  try {
    ...
    if (rows.length > 2e3) return c.json({ error: "Import is limited to 2000 rows at a time" }, 400);
    const clean = rows.map((r) => ({
      ...
      phone: (r.phone || "").trim(),
      title: (r.title || "").trim(),
      status: CONTACT_STATUSES.includes((r.status || "").trim()) ? r.status.trim() : "lead",
    ...
    const companyIds = /* @__PURE__ */ new Map();
    const loadIds = /* @__PURE__ */ __name(async (names) => {
      for (const group3 of chunk(names, LOOKUP_CHUNK)) {
        const placeholders = group3.map(() => "?").join(", ");
    ...
    for (const group3 of chunk(missing, COMPANY_INSERT_CHUNK)) {
      const placeholders = group3.map(() => "(?, ?, ?, ?, ?)").join(", ");
      const params = group3.flatMap((co) => [crypto.randomUUID(), co.name, co.domain, co.industry, co.phone]);
    ...
    let imported = 0;
    for (const group3 of chunk(clean, CONTACT_CHUNK)) {
      const placeholders = group3.map(() => "(?, ?, ?, ?, ?, ?, ?, ?)").join(", ");
      const params = [];
...
app.get("/api/custom-fields", async (c) => {
  ...
  const defs = await listDefs(entity ? entity : void 0);
...
// node_modules/.pnpm/wrangler@4.93.1_@cloudflare+workers-types@4.20260702.1/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// node_modules/.pnpm/wrangler@4.93.1_@cloudflare+workers-types@4.20260702.1/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-Uxv0jc/middleware-insertion-facade.js
...
// node_modules/.pnpm/wrangler@4.93.1_@cloudflare+workers-types@4.20260702.1/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-Uxv0jc/middleware-loader.entry.ts
